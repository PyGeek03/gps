...
'''gps_data.py is a ROS node that parses GPS data and publishes it in a format
other ROS nodes can use

Date Last Updated: 29/9/19 by Marcel Masque

Purpose: Publish GPS data so that other ROS nodes relying on GPS data can
operate correctly
...
def gpsData():
    ...
    # For testing, set an appropriate rate.
    # This wont be needed when there's actual GPS data as the rate of publish
    # will be determined by the GPS data influx rate.
    rate = rospy.Rate(100)
    msg = GPSData()
